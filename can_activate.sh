...
if [ "$CURRENT_CAN_COUNT" -ne "1" ]; then
    if [ -z "$USB_ADDRESS" ]; then
        ...
        for iface in $(ip -br link show type can | awk '{print $1}'); do
            ...
            BUS_INFO=$(echo "$SUDO_PASS" | sudo -S ethtool -i "$iface" | grep "bus-info" | awk '{print $2}')
...
if [ -n "$USB_ADDRESS" ]; then
    ...
    BUS_INFO=$(echo "$SUDO_PASS" | sudo -S ethtool -i "$INTERFACE_NAME" | grep "bus-info" | awk '{print $2}')
...
if [ "$IS_LINK_UP" == "yes" ] && [ "$CURRENT_BITRATE" -eq "$DEFAULT_BITRATE" ]; then
    ...
    if [ "$INTERFACE_NAME" != "$DEFAULT_CAN_NAME" ]; then
        ...
        echo "$SUDO_PASS" | sudo -S ip link set "$INTERFACE_NAME" down
        echo "$SUDO_PASS" | sudo -S ip link set "$INTERFACE_NAME" name "$DEFAULT_CAN_NAME"
        echo "$SUDO_PASS" | sudo -S ip link set "$DEFAULT_CAN_NAME" up
    ...
    echo "$SUDO_PASS" | sudo -S ip link set "$INTERFACE_NAME" down
    echo "$SUDO_PASS" | sudo -S ip link set "$INTERFACE_NAME" type can bitrate $DEFAULT_BITRATE
    echo "$SUDO_PASS" | sudo -S ip link set "$INTERFACE_NAME" up
