...
class GLViewer:
    ...
    def update_normal(self, start, end):
        self.normal_line = (start, end)
    ...
    def draw(self):  
        if self.available:
            ...
            if self.normal_line is not None:
                s, e = self.normal_line

                glLineWidth(3.0)
                glColor3f(1.0, 0.0, 0.0)

                glBegin(GL_LINES)
                glVertex3f(float(s[0]), float(s[1]), float(s[2]))
